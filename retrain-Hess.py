...
for train_num in train_nums:
  ...
  crit_without_hess = CustomLoss2(gradient_weight=1)
  ...
  for w in ws:
    ...
    # Perform forward pass without sparse structure
      path = './Modelos/PointComp/VdPHess' + str(w) + 't' + str(container.ts[i]) + 'Points' + str(train_num) + 'Double.pth'
      net = Net(width=64, depth=2)
      net.load_state_dict(torch.load(path))
      data_train = container.data_train[i]
      data_test = container.data_test[i]
      crit_train = crit2
      #_, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.0001, n_epochs=50, criterion=crit2, crit2=crit3, optimizer_class=opt, l1_weight=w)
      start_time = time.time()  # Start timer
      _, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.00001, n_epochs=1000, criterion=crit_train, crit2=crit_without_hess, optimizer_class=opt, l1_weight=w)
      end_time = time.time()  # Stop timer
      _, loss_test_list2b, extra = set_run(net, data_train, data_test, batch_size=100, lr=0.000001, n_epochs=1, criterion=crit_train, crit2=crit3, optimizer_class=opt, l1_weight=w, crit3 = crit)
      #path_save = './Modelos/VdPTanh/VdPHessSmallTanhL1' + str(w) + 't' + str(container.ts[i]) + 'Weights' + str(a[0]) + '_' + str(a[1]) + '.pth'
      path_save = './Modelos/PointComp/VdPHess' + str(w) + 't' + str(container.ts[i]) + 'Points' + str(train_num) + 'Double.pth'
      torch.save(net.state_dict(), path_save)

print(f'Time per epoch: {(end_time-start_time)/1000}')