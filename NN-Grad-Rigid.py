...
def set_run(net, train_set, test_set, batch_size, lr, n_epochs, criterion, criterion2, optimizer_class, l1_weight=0, crit3=0):
  ...
  for epoch in range(n_epochs):
    ...
    print(f'Epoch: {epoch+1:03} | Train Loss: {train_loss:.04} | Test Loss (H1 Norm): {test_loss:.04} ')
...
crit3 = CustomLoss(gradient_weight=grad_weight)
...
for w in ws:
  ...
  for i in range(len(container.ts)):
    net = Net(width=128, depth=4) # Width*(4 + (Depth-+1)*Width)
    data_train = container.data_train[i]
    data_test = container.data_test[i]
    _, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.0001/grad_weight, n_epochs=50, criterion=crit3, criterion2=crit2, optimizer_class=opt, l1_weight=w)
    _, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.00001/grad_weight, n_epochs=400, criterion=crit3, criterion2=crit2, optimizer_class=opt, l1_weight=w)
    _, loss_test_list2b, extra = set_run(net, data_train, data_test, batch_size=100, lr=0.000001/grad_weight, n_epochs=1, criterion=crit3, criterion2=crit2, optimizer_class=opt, l1_weight=w, crit3 = crit)
    path = './Modelos/RigidTanhL1' + str(w) + 't' + str(container.ts[i]) + 'Grad_W'+ str(grad_weight) + '.pth'
