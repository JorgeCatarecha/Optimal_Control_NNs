...
class Net(nn.Module):
  def __init__(self, width, depth):
    ...
    self.sig = nn.ReLU()  # Tanh activation layer 
...
for w in ws:
  ...
  for i in range(len(container.ts)):
    ...
    _, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.0001, n_epochs=30, criterion=crit2, optimizer_class=opt, l1_weight=w)
    _, _ = set_run(net, data_train, data_test, batch_size=100, lr=0.00001, n_epochs=2000, criterion=crit2, optimizer_class=opt, l1_weight=w)
    _, loss_test_list2b, extra = set_run(net, data_train, data_test, batch_size=100, lr=0.000001, n_epochs=50, criterion=crit2, optimizer_class=opt, l1_weight=w, crit3 = crit)
    ...
    path = './Modelos/VdPTanh/VdPSmallReLUL1' + str(w) + 't' + str(container.ts[i]) + '.pth'
